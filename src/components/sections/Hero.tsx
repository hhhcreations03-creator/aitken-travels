    ...
    <>
      ...
      <section className="relative min-h-[100dvh] overflow-hidden md:hidden">
        ...
        <div className="absolute inset-0 z-[1]" style={{
          background: "linear-gradient(180deg, rgba(10,45,73,0.45) 0%, rgba(10,45,73,0.2) 40%, rgba(10,45,73,0.9) 100%)",
        }} />

        {/* Content — bottom-aligned for a modern app feel */}
        <div className="relative z-[3] min-h-[100dvh] flex flex-col justify-end px-5 pb-6 pt-20">

          {/* Main text block */}
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 0.1 }} className="mb-6">
            <div className="flex items-center gap-2 mb-4">
              <span className="w-5 h-[2px] rounded-full bg-primary-400" />
              <span className="font-mono text-[10px] tracking-[0.18em] uppercase font-semibold text-primary-300">Explore Sri Lanka</span>
            </div>

            <h1 className="font-display font-bold text-white leading-[1.02] tracking-tight text-[32px]">
            ...
            <p className="text-[14px] text-white/70 mt-3 leading-relaxed">
              Cars, vans, buses &amp; bikes — professional drivers, island-wide coverage.
            </p>
          </motion.div>

          {/* CTA Buttons — side by side */}
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.3 }} className="flex gap-3 mb-5">
            <button
              onClick={onOpenBooking}
              className="flex-1 bg-gradient-to-r from-primary-500 to-primary-400 text-white rounded-xl py-3.5 font-semibold text-[14px] cursor-pointer min-h-[48px]"
            >
              Book your ride
            </button>
            <a
              href="#fleet"
              className="flex-1 glass-card text-white rounded-xl py-3.5 font-medium text-[14px] cursor-pointer min-h-[48px] flex items-center justify-center"
            >
              Explore fleet
            </a>
          </motion.div>

          {/* Location bar — glass card with dots + place */}
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.5 }}
            className="bg-white/10 backdrop-blur-md border border-white/10 rounded-xl px-4 py-3 flex items-center justify-between">
            {/* Dots */}
            <div className="flex items-center gap-2">
              {HERO_SLIDES.map((_, i) => (
                <button key={i} onClick={() => { setSlide(i); setProgress(0); }}
                  className="relative w-5 h-5 flex items-center justify-center cursor-pointer bg-transparent border-none" aria-label={`Slide ${i + 1}`}>
                  {i === slide && (
                    <svg width="20" height="20" viewBox="0 0 24 24" className="absolute inset-0 -rotate-90">
                  ...
                  <span className={`block rounded-full transition-all ${i === slide ? "w-1.5 h-1.5 bg-white" : "w-1 h-1 bg-white/40"}`} />
                </button>
              ))}
            </div>
            ...
            <AnimatePresence mode="wait">
              <motion.div key={slide} initial={{ opacity: 0, x: 8 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -8 }} transition={{ duration: 0.25 }}
                ...
                <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="var(--primary-400)" strokeWidth="2.5" strokeLinecap="round">
                  <path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0118 0z" /><circle cx="12" cy="10" r="3" />
                </svg>
                <span className="text-[12px] text-white font-medium">{HERO_SLIDES[slide].place}</span>
                <span className="text-[10px] text-white/40 font-mono">&middot; {HERO_SLIDES[slide].sub.split(" \u00b7 ")[0]}</span>
              </motion.div>
            </AnimatePresence>
          </motion.div>
        </div>
